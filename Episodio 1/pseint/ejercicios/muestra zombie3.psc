Algoritmo MuestraZombie
	...
	Si b=verdadero Entonces
		...
		Si a=verdadero Entonces
			Escribir "La SECUENCIA es válida para su examen"
			Escribir " * * * * ANALIZANDO MATRIZ * * * * "
			salto()
		...
		SiNo
			Si a=Falso Entonces
				Escribir "Pero la SECUENCIA no es compatible (3x3 - 4x4 - 5x5 ETC)" 
				Escribir "Reinicie el algoritmo con una muestra válida"//(No da una raiz cuadrada con numeros enteros)
			FinSi
		FinSi
		
	SiNo
		Si b=Falso Entonces
			Escribir "La muestra no es compatible - Valores fuera de Rango (A-B-C-D) "
			Escribir "Reinicie el algoritmo con una muestra válida "//(No da una raiz cuadrada con numeros enteros)
		FinSi
	FinSi
